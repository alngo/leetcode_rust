pub fn my_atoi(s: String) -> i32 {
    ...
    if chars.len() > 0 {
        ...
        while i < chars.len() {
            let digit = find_digits(chars[i], base);
            if digit.is_some() {
                let limit: Option<i32> = find_limit(num, *digit.as_ref().unwrap(), mult, base);
                if limit.is_some() {
                    return limit.unwrap();
                }
...
fn find_limit(num: i32, digit: i32, mult: i32, base: i32) -> Option<i32> {
    let max_divided: i32 = i32::MAX / 10;
    let max_digit: i32 = i32::MAX % 10;
    if num > max_divided || (num == max_divided && digit > max_digit) {
        return if mult == 1 { Some(i32::MAX)} else { Some(i32::MIN) }
    }
    None
}
